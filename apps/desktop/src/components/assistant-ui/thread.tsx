...
const AssistantMessage: FC<{
  ...
  return (
    <MessagePrimitive.Root
      ...
      ref={enterRef}
    >
      <GaladrielAssistantAvatar running={isRunning} />
...
const StreamStallIndicator: FC = () => {
  ...
  return (
    <StatusRow
      ...
      label={compacting ? COMPACTION_LABEL : 'Galadriel réfléchit'}
    >
      <span aria-hidden="true" className="dither inline-block size-3 rounded-[2px] text-midground/80 animate-pulse" />
      {compacting && <CompactionHint />}
      <ActivityTimerText seconds={elapsed} />
...
function StickyHumanMessageContainer({
  ...
  return (
    ...
    // make assistant-ui/Radix Slot recurse until the renderer trips
    // "Maximum call stack size exceeded". `contents` keeps the DOM child single
    // for Slot while avoiding a layout box that would break sticky positioning.
