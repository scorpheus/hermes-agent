#!/usr/bin/env node
// set-exe-identity.cjs — stamp the Galadriel icon + version metadata onto the
// built Galadriel.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Galadriel.exe keeps the stock Electron icon and
...
//   node scripts/set-exe-identity.cjs <path-to-Galadriel.exe>
//
...
// Stamp the Galadriel icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = path.resolve(__dirname, '..')) {
  ...
  await rcedit(exe, {
    icon,
    'version-string': {
      ProductName: 'Galadriel',
      FileDescription: 'Galadriel Companion App',
      CompanyName: 'Scorpheus',
      LegalCopyright: 'Copyright (c) 2026 Scorpheus'
    }
  })

  console.log('[set-exe-identity] done — Galadriel icon + identity stamped')
}
