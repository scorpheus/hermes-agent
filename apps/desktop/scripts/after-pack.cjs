 ...
 *
 * Stamps the Galadriel icon + identity onto the packed Windows Galadriel.exe via
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Galadriel')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Galadriel'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Galadriel.exe keeps the stock Electron icon`)
  }
}
